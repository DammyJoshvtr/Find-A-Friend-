...
export default function FollowingScreen() {
  ...
  const loadFollowing = async () => {
    setLoading(true)
    try {
      const { data } = await client.models.follows.list({ filter: { follower_id: { eq: id } } }) // TODO: Complex select
      const profiles = (data ?? [])
  ...
  return (
    <SafeAreaView style={[s.container, { backgroundColor: theme.bg }]} edges={['top']}>
      <View style={[s.header, { borderBottomColor: theme.border }]}>
        ...
        <Text style={[s.title, { color: theme.text }]}>Following</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              <Text style={[s.emptyText, { color: theme.textMuted }]}>Not following anyone yet</Text>
            </View>
          }
